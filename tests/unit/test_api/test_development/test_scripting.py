...
def test_eval_code(client):
    client._get.return_value = success_response
    response = client.Scripting.EvalCode("code", "type", "annotations", "params","app_id",)
    client._get.assert_called_once_with(
        "EvalCode",
        params={
            "code": "code",
...
def test_export_scripts(client):
    client._get.return_value = success_response
    response = client.Scripting.ExportScripts(True,"app_id",)
    client._get.assert_called_once_with(
        "ExportScripts",
        params={
            "overwrite": True,
...
def test_get_script(client):
    client._get.return_value = success_response
    response = client.Scripting.GetScript("name","app_id",)
    client._get.assert_called_once_with(
        "GetScript",
        params={
            "name": "name",
...
def test_get_scripts(client):
    client._get.return_value = success_response
    response = client.Scripting.GetScripts("type", 1, 2,"app_id",)
    client._get.assert_called_once_with(
        "GetScripts",
        params={
            "type": "type",
