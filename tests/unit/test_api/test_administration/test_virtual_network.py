...
def test_apply_virtual_networks(client):
    client._get.return_value = success_response
    response = client.VirtualNetwork.ApplyVirtualNetworks(1)
    client._get.assert_called_with("ApplyVirtualNetworks", params={"hostId": 1})
    assert response == success_response
...
def test_get_virtual_networks(client):
    client._get.return_value = success_response
    response = client.VirtualNetwork.GetVirtualNetworks(1)
    client._get.assert_called_with("GetVirtualNetworks", params={"ids": 1})
    assert response == success_response
