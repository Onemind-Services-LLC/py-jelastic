...
def test_add_app(client):
    ...
    response = client.Admin.AddApp("env", "manifest", "ruk")
    client._get.assert_called_with(
        "AddApp",
        params={"envName": "env", "manifest": "manifest", "ruk": "ruk"},
    )
...
def test_edit_app(client):
    ...
    client._get.assert_called_with(
        "EditApp",
        params={"envName": "env", "id": 1, "manifest": "manifest", "ruk": "ruk"},
    )
    assert response == success_response
...
def test_update_app_rating(client):
    ...
    client._get.assert_called_with(
        "UpdateAppRating",
        params={"envName": "env", "id": 1, "rating": 9, "ruk": "ruk"},
    )
    assert response == success_response
