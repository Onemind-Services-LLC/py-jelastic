...
def test_add_private_ssh_key(client):
    ...
    response = client.Account.AddPrivateSSHKey(
        "title",
        "ssh_key"
    )
...
def test_add_public_ssh_key(client):
    client._get.return_value = success_response
    response = client.Account.AddPublicSSHKey(
        "title",
        "ssh_key"
    )
    client._get.assert_called_with(
...
def test_clean_quotas_cache(client):
    client._get.return_value = success_response
    response = client.Account.CleanQuotasCache(
        1
    )
    client._get.assert_called_with(
        "CleanQuotasCache",
...
def test_get_ssh_keys_inner(client):
    client._get.return_value = success_response
    response = client.Account.GetSSHKeysInner(
        1
    )
    client._get.assert_called_with(
        "GetSSHKeysInner",
