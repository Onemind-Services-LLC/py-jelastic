...
def test_get_actions(client):
    client._get.return_value = success_response
    response = client.Tracking.GetActions(CURRENT_DATETIME, [CURRENT_DATETIME, CURRENT_DATETIME])
    client._get.assert_called_with(
        "GetActions",
        params={
            "starttime": CURRENT_DATETIME,
...
def test_get_all_service_name(client):
    ...
    response = client.Tracking.GetAllServiceName(
        [True, True], ["type1", "type2"]
    )
    client._get.assert_called_with(
        "GetAllServiceName",
        params={
            "addServicesWildcard": [True, True],
            "type": ["type1", "type2"]
        },
        delimiter=",",
    )
...
def test_get_env_actions(client):
    client._get.return_value = success_response
    response = client.Tracking.GetEnvActions(CURRENT_DATETIME, [CURRENT_DATETIME, CURRENT_DATETIME],
                                             [1, 1], [1, 1])
    client._get.assert_called_with(
        "GetEnvActions",
        params={
...
def test_get_uid_actions(client):
    client._get.return_value = success_response
    response = client.Tracking.GetUidActions([1, 1], CURRENT_DATETIME, [CURRENT_DATETIME, CURRENT_DATETIME],
...
def test_get_uid_actions_admin(client):
    ...
    response = client.Tracking.GetUidActionsAdmin(
        1, 1, CURRENT_DATETIME, CURRENT_DATETIME, [1, 1], ["servicename1", "servicename2"],
        ["order_field1", "order_field2"], ["order_direction1", "order_direction2"],
...
def test_store_audit_admin_actions(client):
    ...
    client._get.assert_called_with(
        ...
        params={
            "session": "session",
            "trackedevent": {"trackedevent1": "trackedevent1", "trackedevent2": "trackedevent2"}
        }, delimiter=",",
    )
    assert response == success_response
...
def test_store_user_actions(client):
    ...
    response = client.Tracking.StoreUserActions(
        "session", {"tracked_action_event1": "tracked_action_event2",
    ...
    client._get.assert_called_with(
        ...
        params={
            "session": "session",
            "trackedActionEvent": {"tracked_action_event1": "tracked_action_event2",
                                   "tracked_action_event2": "tracked_action_event2"}
        }, delimiter=",",
    )
    assert response == success_response
