...
def test_execute_app_action(client):
    ...
    response = client.Jps.ExecuteAppAction(
        "app_unique_name",["action1","action2"],["settings_id1", "settings_id2"],["params1","params2"],["lang1","lang2"],
    )
...
def test_get_app_info(client):
    ...
    response = client.Jps.GetAppInfo(
        ["jps1","jps2"],["lang1","lang2"],["owner_uid1", "owner_uid2"],
    )
    client._get.assert_called_with(
        "GetAppInfo",
        params={
...
def test_get_app_settings(client):
    ...
    response = client.Jps.GetAppSettings(
        "app_unique_name",["settings_id1", "settings_id2"],["lang1","lang2"],
    )
    client._get.assert_called_with(
        "GetAppSettings",
        params={
...
def test_install(client):
    ...
    response = client.Jps.Install(
        "jps",["env_name1", "env_name2"],["settings1", "settings2"], ["node_group1", "node_group2"], ["display_name1", "display_name2"],
        ["region1", "region2"],["env_groups1", "env_groups2"], [1,1],["logs_path1", "logs_path2"], [True,True],[True,True],
    )
    client._get.assert_called_with(
        "Install",
        params={
